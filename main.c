...
#include "hal-config/hal-config.h" //file produced by .hwconf file
...
static void bluetoothAppTask(void *p_arg)
{
	...
	// Pseudo Code:
	//
	// Configure sensor as gatt host
	// Configure sensor as cte receiver
	//
	// Advertise connection with sensor
	// Wait for connection to tag...
	//
	// Enable cte response on connection (on tag)
	// Send cte_req with sensor
	// Wait for cte packet (tag)...
	//
	// Send cte_response with tag (continuous tx)
	// Wait for cte packet (sensor)...
	//
	// Start iq_sampling after Guard and Reverence Periods
	// Sample iq data (2us)
	//
	//
...
static void proprietaryAppTask(void *p_arg)
{
	...
	RAIL_SetTxPower(railHandle, HAL_PA_POWER);
		...
		//set PHY 1M
		if ( RAIL_BLE_ConfigPhy1Mbps(railHandle) != RAIL_STATUS_NO_ERROR )
		{
			//handle rail_status_t error (https://docs.silabs.com/rail/latest/group-general#gae0d330e2daabbf1988161b4bba1ddef3)
		}
